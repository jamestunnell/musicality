...
[ AddingSequence::Infinite,
  AddingSequence::BiInfinite ].each do |klass|
  describe klass do
    ...
    before :all do
      @pattern = [2,-1,5,-4,3]
      @start_value = 13
      @seq = klass.new(@pattern,@start_value)
    end
      ...
      context 'given offset of 0' do
        it 'should return the start value' do
          [ 0, -3, 7].each do |start_val|
            klass.new(@pattern,start_val).at(0).should eq(start_val)
          end
        end
      end
      ...
      context 'given offset > 0' do
        it 'should keep adding on pattern elements to start_val until the given offset is reached' do
          [1,2,3,5,8,15,45].each do |offset|
            val = @seq.at(offset)
            rep_seq = RepeatingSequence::Infinite.new(@pattern)
...
describe AddingSequence::Infinite do
  before :all do
    @pattern = [2,1,5,4,3]
    @seq = AddingSequence::Infinite.new(@pattern)
  end
...
describe AddingSequence::BiInfinite do
  before :all do
    @pattern = [2,-1,-5,4,3]
    @start_value = 31
    @seq = AddingSequence::BiInfinite.new(@pattern,@start_value)
  end
  ...
  describe '#at' do
    context 'given offset < 0' do
      it 'should keep suctracting pattern elements from start_val until the given offset is reached' do
        [-1,-2,-3,-5,-8,-15,-45].each do |offset|
          val = @seq.at(offset)
          rep_seq = RepeatingSequence::BiInfinite.new(@pattern)
