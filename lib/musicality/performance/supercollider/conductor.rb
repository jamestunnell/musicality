...
SCLANG
  ...
  Score.recordNRT(x, \"BASE_FPATH.osc\", \"BASE_FPATH.aiff\", options: o);
);
SCLANG
  ...
  def write_sc_code bundles, base_fpath
    ...
    File.open(fpath, "w") do |f|
      ...
      f.write SC_HEADER.gsub(/SYNTHDEFS/, synthdefs.map {|s| s.to_sclang }.join("\n\n"))
      f.write bundles.map {|b| "    " + b.to_sclang }.join(",\n") + "\n"
      f.write SC_FOOTER.gsub(/BASE_FPATH/, base_fpath)
    end
    fpath
  end

  def exec_sc_code fpath, verbose
    IO.popen("sclang \"#{fpath}\"") do |io|
      pid = io.pid      
      while response = io.gets
        puts response if verbose
        break if /RESULT =/.match response
      end
